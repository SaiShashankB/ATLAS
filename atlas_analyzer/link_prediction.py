...
def create_node_features(graph):
    """Creates one-hot encoded features for the first and last letter."""
...
def run_node2vec_link_prediction(graph):
    print("\n" + "="*20 + " Part 1: Link Prediction with Node2Vec " + "="*20)
    ...
    n2v = Node2Vec(train_graph, dimensions=64, walk_length=30, num_walks=200, workers=4, quiet=True)
...
def run_simple_gnn_link_prediction(pyg_data):
    print("\n" + "="*20 + " Part 2: Link Prediction with Simple GNN " + "="*20)
    ...
    gnn = SimpleGNN(in_channels=pyg_data.num_node_features, out_channels=64)
    predictor = LinkPredictor(in_channels=64, hidden_channels=64, out_channels=1)
